...
- **`run <goal>`** — a supervised run of `<goal>` with autonomy armed (halt, alerts, unattended = deny, 75% compaction window). The default is the FOREGROUND: a normal interactive session in the person's own terminal, which they watch and can interject in. A session cannot open a second terminal UI, so print the exact command for them to run and stop there:
  `<engram venv python> -m claude_engram.run --project "<cwd>" --goal "<goal>"` — add `--model <alias>`, `--prompt "<task text>"`, `--alert-command "<cmd with {message}>"` when given. Only when they say "in the background" or "headless": run it yourself with `Bash(run_in_background=true)` adding `--headless` (and `--max-turns N`); relay the session id and say the run reports when it ends. Never start a second run for the same goal.
