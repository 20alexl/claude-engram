...
- **`run <goal>`** (optionally `check: <shell command>` and `max turns: N` in the text) — arm a supervised run IN THIS SESSION, like `/goal` or `/loop`: call `session_mine(run_start, project_path=<cwd>, goal=<goal>, check=<command or omitted>, max_turns=<N or omitted>)`, then start working toward the goal in this same turn. From the next stop engram re-prompts you with the goal until the check command exits 0 (the truth; engram never judges the sentence), or `session_mine(run_done, evidence=...)` when there is no check (recorded as self-declared), or the turn cap, or the strike-cap halt, or `run_stop`. While it runs, autonomy mode is on: ask-first rules refuse their commands, three no-effect strikes halt, alerts go out, and the person can interject or stop at any time. You may start a run yourself when the person's plan says to execute unattended and the goal is checkable; say so first. Never start a second run in a session that has one running.
- **`stop`** — `session_mine(run_stop, evidence=<why>)`. **`done`** — `session_mine(run_done, evidence=<what proves it>)`.
- For cron and overnight, outside any session: `python -m claude_engram.run --headless --goal "<goal>"` (the launcher; `--dry-run` prints the plan).
