        ...
        inputSchema={
            ...
            "properties": {
                "operation": {
                    ...
                    "description": "Operation to perform. run_report: write and return this session's auditable run report (<project>/.engram/runs/) -- hook-captured facts only: commits, turns, files with edit counts, tests, errors, compactions with sizes and what each restored, checkpoints deliberate vs auto, goal text. run_start (goal, check, max_turns): arm /engram run IN THIS SESSION -- engram re-prompts at every stop until the check command exits 0 (or run_done without a check), the turn cap, the strike-cap halt, or run_stop; autonomy mode (halt, alerts, unattended=deny) is on meanwhile. run_done (evidence): declare the goal met. run_stop / run_status.",
                },
                "goal": {"type": "string", "description": "For run_start: the goal in one sentence; it becomes the directive at every stop"},
                "check": {"type": "string", "description": "For run_start: a shell command that exits 0 when the goal is met (the truth; engram never judges the sentence). Optional; without it the run ends only on run_done"},
                "max_turns": {"type": "integer", "description": "For run_start: hard turn cap (default 150)"},
                "evidence": {"type": "string", "description": "For run_done / run_stop: what proves it, or why"},
                "project_path": {"type": "string", "description": "Project directory"},
                "query": {
                    "type": "string",
