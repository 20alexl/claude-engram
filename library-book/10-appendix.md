...
- **A process census in `claude_engram_status`** (`procs.py`): every engram process by role, pid, memory, age and store, with a warning at a second scorer or miner on the same store. The census also found that every smoke-test run had been leaving a 1.2 GB daemon behind for a pytest temp store; the tests set `CLAUDE_ENGRAM_NO_DAEMON` now.
...
- **Correction cue.** The broad list scored precision 0.74, recall 0.57; the narrow one (no, not, don't, never, wrong, instead, stop, rather than, meant, prefer, differently, always, avoid, only) 0.97 / 0.80. Word position did not matter, so there is no position rule. "undo" and "revert" are commands more often than corrections and moved to the decision side. A hedge ("not sure", "wondering", "whether") is never a correction.
- **Decision cue.** 0.85 / 0.79 before, 1.00 / 0.95 after: choice verbs added (implement, pick, choose, commit to, settle on, standardize on, lock in, go with, move to, needs to, "X, not Y"); an edit verb (rename, replace, move, split, migrate, swap, convert, upgrade, switch) counts as a decision when it names a transition (from A to B, with, using, over) or a scope (all, every, from now on) and its object is not deictic ("this file", "the last commit"); hedges, history, opinion and third parties are excluded ("might", "used to", "were going to", "personally", "most teams", "our competitors", "the previous team", "use case"); acknowledgements strung together ("ok looks good, thanks") are one acknowledgement; the minimum is three words.
- Two shapes the corpus lacks, found by running the extractors in-process over the tail of a long transcript (a measurement, no store written): a message that starts with markup (a pasted wrapper tag) is code-shaped, and a request opener ("can you", "give me", "let me know", "what should") asks rather than decides. Both rejected; the corpus scores stay at their floors (decision 1.00 / 0.93). Over 82 typed prompts the miner would now store 4 decisions and no preferences, against roughly 90 a day before.
- Migration `0.8.47:prune_junk_decisions_retuned` re-runs the pruning with this gate (idempotent; on the live store 59 more archived, 354 kept). The semantic threshold on the correction path is left as it was: on the corpus no threshold or margin separates corrections from not-decisions, so the form gate is the filter and the number is documentation.

### v0.8.46 (2026-09-22)

An audit of eight days of one long session (1,005 prompts, 23 compactions, 170 deliberate checkpoints), read from its transcript, plus the notes of the model running it. What carried: every compaction restored a checkpoint under twenty minutes old, both auto-compactions were preceded by a CHECKPOINT NOW and a save, the pressure and budget nudges fired at the right numbers, the rule detector's 28 hits were all real, and engram cost about 60 tokens a prompt. What was noise, in order of volume, each fixed by its FORM, never by any one user's phrasing:

- **Captured decisions have to have the shape of one.** The miner's question filter (`text.rstrip("?").endswith("?")`) could never be true, and its semantic tier stored anything above a bare cosine to the decision templates, which is where most short text sits under a modern encoder: 636 "decisions" in one week, then injected before edits as "Relevant memories for this file", one of them nineteen times. `mining/decision_gate.py` is shared by the miner, the prompt hook and a migration: a decision is a declarative sentence of four or more words, not a question, not an acknowledgement, not a count or a table row or a commit report, not mostly symbols, and it carries a deciding word. The miner's semantic tier scores a margin over category templates (a question, an instruction, an approval, a status report, a count, a pasted log). A "(confirmed) <proposal>" entry is judged on the proposal's first sentence. The correction extractor's substring list ("not" inside "note", "use" inside "because") is replaced by the same gate. Migration `0.8.46:prune_junk_decisions` archives, never deletes, the stored ones that fail it: 1,569 of 2,042 machine-captured decisions on the live store, restorable by id; manual entries untouched.
- **Test tracking said "baseline established" 110 times and never once "flip".** Two causes. The Stop hook cleared the test status every turn (0.8.38); the state file is one per session, so the status now stays. And a chain that reads a log and does anything else (`cat run.log; bash count.sh`, `tail -3 out.txt; date`) was trusted because one segment could run something, so the marker in the cat counted (29 of the 110). `_segment_kind` sorts every segment into run / read / noise; the output is trusted when the command names a test runner, or when nothing in the chain merely reads.
- **Milestone nudges: 41 sent, 18 were closes.** Six shapes are rejected now, measured over those 41: a table row; a sentence still in motion (waiting, running, queued, a percentage through, time to go); reported speech (says, repeats, told); a rule or a definition (means, only if); a number right next to "passed" or "closed" (a count, a price); and a participle used as an adjective after an article ("the merged tree exceeded"). Precision on the sample 0.44 to 0.85, no true close lost among them.
- **Recurring errors were stale under a fresh date.** Errors were grouped by a signature that templated every identifier away, so all missing-file errors were one pattern; its example was the longest instance ever seen (a week-old path, fixed since) and its `last_seen` was fresh because some other file went missing. The group is now the concrete error (directories, hashes and numbers templated, identifiers and file names kept), and the example and fix come from the latest sighting.
- **A code exception attached to a plan file.** The miner tied a mistake to whatever the assistant edited next, and a markdown plan then warned "Watch for: TypeError" on every edit. A mined mistake attaches to the files its traceback names, else to code files only; and the pre-edit prediction never names an exception class for a non-code file, whatever old data says.
- **The last-session line was workspace-wide.** A root-cwd start reported every project's files and errors ("1,089 files, 405 tool errors"). The banner narrows to the sub-project most of that session's edits belong to and says which ("mostly trade-lab").
- **The prompt hook re-ran the full banner mid-session.** Twelve times in eight days: it took four quiet hours for a new session. A session started once stays started.
- **"2,893 commits on other local branches."** `rev-list --branches --not <commit>` counts the whole history of every branch that diverged before the checkpoint. Bounded with `--since=<checkpoint time>`, the same line reads 3.
- **Memory lines cut mid-word at 80 characters.** A read-context or pre-edit memory now shows up to 200 characters, cut at a sentence or a word.
