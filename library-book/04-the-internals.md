...
- `_auto_capture_from_prompt()` keeps only the prompt pre-filter (length, a slash command, pasted markup); the judgement is `hooks/intent.capture_decision(text, server_only=False)`: semantic via the scorer server (or an in-process model unless `server_only`) → regex over each sentence → `CAPTURE_THRESHOLD` (0.6) → the shape gate (`mining/decision_gate.looks_like_decision`) → the sentence cut at a word to 300 chars, or `""`. The session miner's preference path calls the same function (`server_only=True`, it runs inside the MCP server), so a sentence is stored by one rule wherever it was seen
...
- `compaction_point(window, project_dir, session_id)`: `CLAUDE_CODE_AUTO_COMPACT_WINDOW` (plain count, ≥100K), else the `--autocompact` launch flag of a background job (`job_autocompact`: `~/.claude/jobs/<first 8 of session id>/state.json` → `respawnFlags`, matched on the file's `sessionId`/`resumeSessionId`; source `launch flag`), else `autoCompactWindow` from `.claude/settings.local.json` → `.claude/settings.json` → `~/.claude/settings.json` (any documented form), else the model default: the 200K boundary, or 967K on a window above 200K. Capped at the window. An interactive session's launch flag is invisible from a hook and reads as `model-default`
...
- `note_stop(state, last_message)` in `context_pressure` stages `milestone_pending` when a claim has no deliberate checkpoint this turn (compared against the previous Stop time). Subagent stops are ignored. A claim also resets the fallback counter: it is a unit boundary. Before the claim check, `_turn_banked_with_remember` reads the turn's `records` (`stall.note_tool` keeps `name:op` per record call, and marks `remember_state` when the remembered text reads as resume state): a `memory:remember` with no `context:checkpoint_save`, plus a close or state-like text, stages the `claim_remember` kind, whose text says a remember is a fact and the restore reads checkpoints only. Never rate-limited
...
- `_hook_pre_tool` (PreToolUse, matcher `""`, daemon-served): one state read; not halted: no output. Halted: `permissionDecision: deny` with `deny_reason()` (names the release CLI and the two open calls) for every tool except `HALT_ALLOWED_TOOLS` = PushNotification, `mcp__claude-engram__context`, ToolSearch, SendMessage (PushNotification is deferred on the newest models and needs its schema loaded, seen on the first live halt; SendMessage is a halted subagent's only way to report). A call whose stdin carries `agent_id` is never denied: subagents share the session id and the halt is for the main loop. `note_denied()` counts. `close_turn()` returns `halted` afterwards, so denied attempts are not more strikes
