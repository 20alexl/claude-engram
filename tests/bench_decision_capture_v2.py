...
CORPUS = [
    ...
    # --- Approval verdicts (10) ---
    # A verdict on a plan under discussion: the plan is not in the sentence,
    # so the entry would say only that something was approved (2026-09-25).
    ("the caching plan is approved", False, "approval_verdict", "plan approved"),
    ("approve the three fixes and the rename", False, "approval_verdict", "approve list"),
    ("both proposals approved", False, "approval_verdict", "both approved"),
    ("approved as written", False, "approval_verdict", "as written"),
    ("plan accepted, no changes", False, "approval_verdict", "accepted"),
    ("the second draft is approved", False, "approval_verdict", "draft approved"),
    ("green light on the parser rewrite", False, "approval_verdict", "green light"),
    ("sign-off on the schema change", False, "approval_verdict", "sign-off"),
    ("approve all four items", False, "approval_verdict", "approve items"),
    ("the fix set is approved, proceed", False, "approval_verdict", "approved + proceed"),
    # --- Hedged leanings (10) ---
    # An opinion held loosely: a leaning, a guess, a choice left open.
    ("I think we should probably use the queue here", False, "hedge", "probably"),
    ("leaning toward postgres but open to it", False, "hedge", "leaning toward"),
    ("either works for me, up to you", False, "hedge", "up to you"),
    ("I'd guess the lockfile is the issue", False, "hedge", "I'd guess"),
    ("no strong opinion, redis is fine I suppose", False, "hedge", "I suppose"),
    ("we could go with sqlite for now, or not", False, "hedge", "or not"),
    ("maybe keep the old parser, not sure yet", False, "hedge", "maybe + not sure"),
    ("I guess the smaller model would do", False, "hedge", "I guess"),
    ("possibly the retry is what we want here", False, "hedge", "possibly"),
    ("if you think so, use the cache layer", False, "hedge", "if you think so"),
    # --- Questions without a question mark (10) ---
    ("would it be better to split the module", False, "unmarked_question", "would it be better"),
    ("what is the right default for the timeout", False, "unmarked_question", "what is"),
    ("is the cache still needed after the rewrite", False, "unmarked_question", "is the"),
    ("do we still want the retry on the client", False, "unmarked_question", "do we still"),
    ("should the parser keep the old flag", False, "unmarked_question", "should the"),
    ("any reason not to use the queue here", False, "unmarked_question", "any reason"),
    ("which one do you prefer for the store", False, "unmarked_question", "which one"),
    ("how would you handle the retry instead", False, "unmarked_question", "how would you"),
    ("does the daemon always need the lock", False, "unmarked_question", "does the"),
    ("are we keeping the legacy endpoint", False, "unmarked_question", "are we"),
    # --- Commands (10) ---
