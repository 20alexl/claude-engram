...
TEST_CASES = [
    ...
    (
        ...
        ["migration"],  # mistakes/rules for db/ ranked higher than discovery about indices
        ["React", "OAuth"],
        "same dir: db models (rules dominate over discoveries)",
    ),
    (
        "api/middleware.py", ["api"],
    ...
    # --- Tag-only relevance ---
    (
        "services/auth_service.py", ["auth", "security"],
        ["auth"],
        ["React", "Tailwind"],
...
def run_benchmark():
    ...
    for file_path, tags, must_contain, _, desc in TEST_CASES:
        ...
        reader_results = reader.get_scored_memories(
            project, {"file_path": file_path, "tags": tags}, limit=3,
        )
    ...
    # Summary
    contain_rate = found_must_contain / total_must_contain * 100 if total_must_contain else 0
    ...
    print(f"  Contain:   {found_must_contain}/{total_must_contain} keywords found ({contain_rate:.0f}%)")
    print(f"  Exclude:   {clean_must_not}/{total_must_not} keywords excluded ({exclude_rate:.0f}%)")
