...
check("from . import x (module)", "a.b.x" in _import_targets(["from . import x"], "a.b.c", False))
check("from .d import y (module)", "a.b.d" in _import_targets(["from .d import y"], "a.b.c", False))
check("from .. import z (module)", "a.z" in _import_targets(["from .. import z"], "a.b.c", False))
# package a.b (__init__): '.' is a.b itself
check("from . import x (package)", "a.b.x" in _import_targets(["from . import x"], "a.b", True))
check("absolute from", "pkg.mod" in _import_targets(["from pkg.mod import T"], "x.y", False))
...
check("base imported by mid+top", idx.dependents_of("pkg.base") == ["pkg.mid", "pkg.top"])
check("mid imported by top", idx.dependents_of("pkg.mid") == ["pkg.top"])
check("top is a leaf", idx.dependents_of("pkg.top") == [])
check("module_for_file", (idx.module_for_file(str(root / "pkg" / "base.py")) or {}).get("module_path") == "pkg.base")
...
check("leaf module silent", pc.blast_radius(str(root / "pkg" / "top.py"), str(root)) == "")
...
check("reverse map rebuilt on reload", idx2.dependents_of("pkg.base") == ["pkg.mid", "pkg.top"])
