...
| **In-session run** | `session_mine(run_start/run_stop/run_done/run_status)` via `/engram run|stop|done`; Stop (the loop) | `hooks/autorun.py`: `run.auto` in the session state; while running, Stop answers `{"decision":"block","reason":<directive>}` until the check command exits 0 (or `run_done` without a check = self-declared), the turn cap, the halt, or `run_stop`; autonomy mode is on meanwhile (`autonomy_on(state)`); end = one alert + the run report's "Engram run" section |
