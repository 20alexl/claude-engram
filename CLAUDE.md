...
The distance is computed from the statusline's token counts (hooks receive none) against the actual compaction point: `CLAUDE_CODE_AUTO_COMPACT_WINDOW`, else a background job's `--autocompact` launch flag (read from its saved `respawnFlags`), else `autoCompactWindow` in settings, else the model default (200K boundary, or ~967K on a native-1M model). Never the raw percentage.
