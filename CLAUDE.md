...
| **Launcher** | `python -m claude_engram.run` / `/engram run` | Manifest, `--session-id`, `CLAUDE_CODE_AUTO_COMPACT_WINDOW` at 75% of the model's window, `--max-turns`, park hint, `claude -p --output-format json`; a `rate_limit` exit with a known reset sleeps and `--resume`s the same session (max 3); run report + closing alert |
