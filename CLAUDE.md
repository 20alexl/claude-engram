...
| **Decision capture** | UserPromptSubmit | "let's use X" parsed via semantic + regex scoring, then the shape gate shared with the miner (`mining/decision_gate.py`): a declarative sentence of four or more words, no question, not an acknowledgement, count, table row or commit report, with a deciding word; hedges, history and third parties excluded; an edit verb only with a transition or a scope. The miner scores a margin over category templates and applies the same gate; migrations `0.8.46`/`0.8.47:prune_junk_decisions*` archived what was stored before it. Tuned and pinned on the neutral 220-prompt corpus by `tests/bench_correction_gate.py` (decision 1.00 / 0.95, correction 0.97 / 0.80), never on a session's own sentences |
