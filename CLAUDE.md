...
| **Decision capture** | UserPromptSubmit | "let's use X" parsed via semantic + regex scoring, then the shape gate shared with the miner (`mining/decision_gate.py`): a declarative sentence of four or more words, no question, not an acknowledgement, count, table row or commit report, with a deciding word. The miner scores a margin over category templates and applies the same gate; migration `0.8.46:prune_junk_decisions` archived what was stored before it |
