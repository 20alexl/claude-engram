...
|---|---|
| Session start | Prints the banner: the project's rules (inherited ones marked), its own past mistakes with a count of the pooled ones, the latest deliberate checkpoint with its `/goal` and how far the repo moved since, last session's files and activity, recurring errors with their known fixes, and the project's known-good test commands that currently pass. Runs data migrations. On a resume or after a compaction every block is scoped to the project the session's own edits name, read from the transcript, so a session run from a workspace root sees its project's errors, test commands and last session, not a sibling's. Engram's own failures never appear as a project's recurring errors. On a project's first session, finds existing Claude Code history and mines it in the background. Seeds the default pack once (see [Project defaults](#project-defaults)) and announces a pending rotation. Warns when Claude Code's transcript format is no longer recognized, and when no statusline records context usage |
| Every prompt | Captures decisions from what you type ("let's use X", "switch to Y", "from now on always Z") by semantic scoring with a regex fallback. Questions and requests for information are not captured. Delivers any staged nudge |
| Before an edit | Injects the three memories most relevant to the file (a rule only when it names the file, an entry older than a month only when it names the full path), warns before a past mistake tied to that file, warns after three edits to the same file in one session (edit loop), verifies that a proposed import resolves against the per-project code index (`<engram-precheck>`, with the closest name), lists the modules that import the file (`<engram-blast-radius>`), shows related files and likely errors from history, and checks the declared task scope |
...
| After a shell command | Tracks test runs and says so only on the first result and on each flip (a read-only command whose output quotes a test line is never a test run), warns after three failed searches in a row (search spiral), and records which injections preceded a passing test (the outcome feedback loop) |
