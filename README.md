...
```text
/engram run <goal>                       # from a session: launches a separate headless run in the background
python -m claude_engram.run --goal "<condition>" --prompt "<task>" --model sonnet --max-turns 150 \
    --alert-command 'curl -s -d {message} https://ntfy.sh/<topic>'
```

The launcher writes a manifest (goal, model, permission mode, the rules in scope with their detectors, the start commit), sets `CLAUDE_CODE_AUTO_COMPACT_WINDOW` to 75% of the model's window (a number that fits the model, not a fixed count), sets a hard `--max-turns` (the goal evaluator's own "or stop after N turns" is not honored), tells the model to park on a wait primitive rather than poll, and runs `claude -p` with a session id it chose. On exit it reads the session's state: a usage-limit failure with a known reset sleeps until the window resets and resumes the same session (a resume restores an active goal); anything else ends the run. Then it writes the run report and sends the closing alert.
